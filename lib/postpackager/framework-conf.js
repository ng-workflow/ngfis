...
function analysisComponentModulesAlias(componentFile, map, ret){
  var componentRoot = fis.config.get('settings.command.install.directory').replace(/\\/g, '/').replace(/\/$/, '').replace(/^\//, '') || 'component_modules';
...
module.exports = function (ret, conf, settings, opt){
  ...
  //makeComponentModulesAlias(ret.src['/bower.json'], map, ret);
  ...
  fis.util.map(ret.src, function(subpath, file){
    var id = file.getId();
    if(['component.json', 'bower.json', '.bower.json','package.json'].indexOf(file.basename.toLowerCase()) !== -1){
      file.release = false;
  ...
  //提供cache参数的时候，特殊处理
  if(map.cache){
    var jsCallback = map.defineJSCallback || 'require.define';
    var cssCallback = map.defineCSSCallback || 'require.defineCSS';
    fis.util.map(ret.src, function(subpath, file){
      if(file.isMod) {
        ...
        if(file.isCssLike){
          ...
          //原css不发布
          file.release = false;
        }else if(file.isViews) {
          file.release = false;
          //var mainFileName = file.fullname.replace(/\.tpl\.html$/, '.js');
          //if(fis.util.isFile(mainFileName)){
          //  file.release = false;
          //}else{
          //  //生成对应的js文件
          //  var content = jsCallback + "('" + file.getId() + "', function(require, exports, module){\n\nexports.template = "
          //    + JSON.stringify(file.getContent()) + ";\n"
          //    + "require('modules/app').registerTemplate('" + file.basename + "', exports.template);"
          //    +"\n\n});";
          //  var f = fis.file(file.realpath);
          //  f.setContent(content);
          //  f.compiled = true;
  ...
  //var file = fis.file(fis.project.getProjectPath(), 'private/framework.json');
